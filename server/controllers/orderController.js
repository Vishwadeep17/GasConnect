...
const Station = require("../models/gasStation.js");
...
exports.getOrderById = async (req, res) => {
    ...
    try {
        ...
        if (!order) {
            return res.status(400).json({ message: "Order with that id does not exist" });
        }
        res.status(200).json(order);
    } catch (error) {
        res.status(400).json(error);
...
exports.getOrderByGasStationId = async (req, res) => {
    let order;
    try {
